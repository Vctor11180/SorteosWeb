...
function createTransaction($db, $usuarioId) {
    try {
        ...
        $metodosPermitidos = ['PayPal', 'Transferencia', 'Visa', 'Saldo Interno', 'Pago Móvil', 'Zelle', 'Binance', 'Efectivo'];
        ...
        // Normalizar números de boletos (convertir enteros a strings con padding)
        $numerosBoletosNormalizados = [];
        foreach ($numerosBoletos as $num) {
            // Convertir a entero si es string, luego a string con padding
            $numInt = is_numeric($num) ? intval($num) : null;
            if ($numInt !== null && $numInt > 0) {
                $numerosBoletosNormalizados[] = str_pad(strval($numInt), 4, '0', STR_PAD_LEFT);
            }
        }
        
        if (empty($numerosBoletosNormalizados)) {
            $db->rollBack();
            sendError('No se pudieron normalizar los números de boletos', 400);
            return;
        }
        ...
        $stmt->execute([
            ...
            ':gateway_reference' => $referenciaPago ?: null,
            ':comprobante_url' => $comprobanteUrl ?: null
        ]);
        
        $idTransaccion = $db->lastInsertId();
        
        // Asociar boletos a la transacción y actualizar estado a "Vendido"
        $stmt = $db->prepare("
        ...
        foreach ($boletosValidos as $boleto) {
            ...
            // Actualizar estado del boleto a "Vendido" y asociar con la transacción
            // Intentar actualizar id_transaccion_reserva si el campo existe
            // Primero intentar con id_transaccion_reserva
            try {
                $stmtUpdate = $db->prepare("
                    UPDATE boletos
                    SET estado = 'Vendido',
                        id_usuario_actual = :usuario_id,
                        id_transaccion_reserva = :id_transaccion
                    WHERE id_boleto = :id_boleto
                ");
                $stmtUpdate->execute([
                    ':usuario_id' => $usuarioId,
                ...
                // Si falla por columna no encontrada (42S22), intentar sin id_transaccion_reserva
                if ($e->getCode() == '42S22' || strpos($e->getMessage(), 'Unknown column') !== false) {
                    error_log("Campo id_transaccion_reserva no encontrado, actualizando sin él");
                    $stmtUpdate = $db->prepare("
                        UPDATE boletos
                        SET estado = 'Vendido',
                            id_usuario_actual = :usuario_id
        ...
        $errorMsg = "Error PDO en createTransaction: " . $e->getMessage();
        $errorMsg .= "\nSQL Error Code: " . $e->getCode();
        if (isset($stmtUpdate) && $stmtUpdate) {
            $errorMsg .= "\nUpdate Query: " . $stmtUpdate->queryString;
        }
        if (isset($stmt) && $stmt) {
            $errorMsg .= "\nInsert Query: " . $stmt->queryString;
        }
        error_log($errorMsg);
        error_log("Stack trace: " . $e->getTraceAsString());
        
        // Enviar el mensaje de error al cliente para debugging (luego se puede ocultar)
        $debugMsg = 'Error al crear la transacción: ' . $e->getMessage();
        if ($e->getCode() == '42S22') {
            $debugMsg .= ' (Columna no encontrada - verifica que id_transaccion_reserva exista en la tabla boletos)';
        }
        sendError($debugMsg, 500);
    } catch (Exception $e) {
        if ($db->inTransaction()) {
            $db->rollBack();
        }
        $errorMsg = "Error general en createTransaction: " . $e->getMessage();
        error_log($errorMsg);
        error_log("Stack trace: " . $e->getTraceAsString());
