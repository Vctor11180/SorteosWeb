...
async function loadSorteoData() {
    try {
        ...
        const response = await fetch(`../api/api_sorteos.php?action=get_details&id=${currentSorteoId}`);
...
async function loadTicketStats() {
    ...
    try {
        const response = await fetch(`../api/api_boletos.php?action=get_available&id_sorteo=${currentSorteoId}`);
...
async function handleAssignTickets() {
    ...
    if (btnAsignar) {
        ...
        try {
            ...
            const response = await fetch('../api/api_boletos.php?action=assign_random', {
                method: 'POST',
                headers: {
...
function displayAssignedTickets(data) {
    // data puede venir con boletos_asignados (array de números) o boletos (array de objetos)
    const boletosObjetos = data.boletos || [];
    const boletosNumeros = data.boletos_asignados || [];
    const numeros = data.numeros_boletos || [];
    
    // Si tenemos objetos completos, usarlos; si no, crear objetos desde los números
    let boletos = [];
    if (boletosObjetos.length > 0) {
        boletos = boletosObjetos;
    } else if (boletosNumeros.length > 0) {
        // Crear objetos desde los números
        boletos = boletosNumeros.map((num, index) => ({
            numero_boleto_int: num,
            numero_boleto: numeros[index] || String(num).padStart(4, '0')
        }));
    }
    ...
    // Mostrar boletos en la sección destacada con botones de eliminación
    if (assignedDisplay) {
        assignedDisplay.innerHTML = boletos.map((boleto) => {
            const numeroInt = boleto.numero_boleto_int || parseInt(boleto.numero_boleto);
            const numeroDisplay = boleto.numero_boleto || String(numeroInt).padStart(4, '0');
            return `<div class="bg-primary/20 border-2 border-primary/40 rounded-xl p-4 text-center transform hover:scale-105 transition-transform relative group">
                <button type="button" onclick="removeTicketFromCart(${numeroInt}, '${numeroDisplay}')" class="absolute top-2 right-2 hover:bg-red-500/20 text-red-400 hover:text-red-300 rounded p-1 transition-colors" title="Eliminar boleto">
                    <span class="material-symbols-outlined text-[16px]">close</span>
                </button>
                <p class="text-xs text-text-secondary mb-1">Boleto</p>
                <p class="text-2xl font-black text-primary">#${numeroDisplay}</p>
            </div>`;
        }).join('');
    }
    ...
    // Actualizar contenedor de boletos en el footer con botones de eliminación
    if (selectedContainer) {
        selectedContainer.innerHTML = boletos.map((boleto) => {
            const numeroInt = boleto.numero_boleto_int || parseInt(boleto.numero_boleto);
            const numeroDisplay = boleto.numero_boleto || String(numeroInt).padStart(4, '0');
            return `<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-sm font-medium bg-primary/10 text-primary border border-primary/20 group relative">
                #${numeroDisplay}
                <button type="button" onclick="removeTicketFromCart(${numeroInt}, '${numeroDisplay}')" class="ml-1 hover:text-red-400 hover:bg-red-500/20 rounded p-0.5 transition-colors" title="Eliminar boleto">
    ...
    const ticketIds = boletos.map(b => b.numero_boleto_int || parseInt(b.numero_boleto));
    ...
    // Actualizar variable global assignedTickets
    assignedTickets = ticketIds;
...
async function checkMyAssignedTickets() {
    ...
    try {
        const response = await fetch(`../api/api_boletos.php?action=get_my_assigned&id_sorteo=${currentSorteoId}`);
        const text = await response.text();
        const data = JSON.parse(text);
        ...
        if (data.success && data.data && data.data.boletos && data.data.boletos.length > 0) {
            ...
            // Mostrar boletos asignados (pasar objetos completos)
            displayAssignedTickets({
                boletos: boletos, // Pasar objetos completos
...
async function checkAvailabilityBeforeAssign(cantidad) {
    ...
    try {
        const response = await fetch(`../api/api_boletos.php?action=check_availability&id_sorteo=${currentSorteoId}&cantidad=${cantidad}`);
        const text = await response.text();
        const data = JSON.parse(text);
...
async function removeTicketFromCart(numeroInt, numeroDisplay) {
    if (!currentSorteoId) {
        customAlert('Error: No se encontró información del sorteo.', 'Error', 'error');
        return;
    }
    ...
    // Intentar usar customConfirm si está disponible
    if (typeof customConfirm === 'function') {
        try {
            confirmed = await customConfirm(
                `¿Estás seguro de que deseas eliminar el boleto #${numeroDisplay}?`,
                'Eliminar Boleto',
                'warning'
            );
        } catch (e) {
            console.warn('Error al usar customConfirm, usando confirm nativo:', e);
            confirmed = confirm(`¿Estás seguro de que deseas eliminar el boleto #${numeroDisplay}?`);
        }
    } else {
    ...
    try {
        console.log('Eliminando boleto:', { numeroInt, numeroDisplay, currentSorteoId });
        
        // Llamar a la API para liberar el boleto
        // La API espera números enteros, los normaliza internamente
        const response = await fetch('../api/api_boletos.php?action=release', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json'
            ...
            body: JSON.stringify({
                id_sorteo: currentSorteoId,
                numeros_boletos: [numeroInt] // Enviar número entero, la API lo normaliza
            })
        });
        ...
        if (!response.ok || !data.success) {
            throw new Error(data.error || data.message || 'Error al liberar el boleto');
        }
        
        console.log('Boleto liberado exitosamente:', data);
        
        // Recargar los boletos asignados desde la API para actualizar la vista
        await checkMyAssignedTickets();
        
        // Actualizar estadísticas
        await loadTicketStats();
        
        if (typeof customAlert === 'function') {
            customAlert(`Boleto #${numeroDisplay} eliminado exitosamente.`, 'Boleto Eliminado', 'success');
        }
        
    } catch (error) {
        console.error('Error al eliminar boleto:', error);
        if (typeof customAlert === 'function') {
            customAlert('Error al eliminar el boleto: ' + error.message, 'Error', 'error');
        } else {
...
window.removeTicketFromCart = removeTicketFromCart;
