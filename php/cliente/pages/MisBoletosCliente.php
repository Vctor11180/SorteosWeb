...
<body class="bg-background-light dark:bg-background-dark font-display overflow-hidden h-screen flex text-white">
    ...
    <main class="flex-1 flex flex-col min-w-0 bg-[#111318]">
        <!-- Top Header -->
        <header
            class="h-16 flex items-center justify-between px-6 lg:px-10 border-b border-[#282d39] bg-[#111318] sticky top-0 z-20">
            <!-- Mobile Menu Toggle -->
            ...
            <div class="flex flex-col gap-8">
                ...
                <div class="flex flex-wrap gap-2 items-center text-sm">
                    <a class="text-text-secondary hover:text-primary transition-colors"
                        href="DashboardCliente.php">Inicio</a>
                ...
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div class="flex flex-col gap-2">
                        ...
                        <p class="text-text-secondary text-base max-w-2xl">Consulta el estado de tus participaciones,
                            descarga tus comprobantes y verifica si eres un ganador.</p>
                    </div>
                </div>
                    ...
                    <div class="flex flex-col lg:flex-row gap-4">
                        ...
                        <div class="flex-1">
                            <label class="flex flex-col gap-1.5 w-full">
                                <span
                                    class="text-xs font-semibold uppercase text-text-secondary tracking-wider">Buscar</span>
                                <div class="relative group">
                                    ...
                                    <input
                                        class="w-full h-11 pl-10 pr-4 bg-[#111318] border border-[#282d39] rounded-lg text-sm text-white placeholder:text-text-secondary focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all"
                                        placeholder="Buscar por número de boleto..." type="text" />
                                </div>
                            </label>
                        </div>
                        <!-- Filter: Raffle -->
                        <div class="w-full lg:w-64">
                            <label class="flex flex-col gap-1.5 w-full">
                                <span
                                    class="text-xs font-semibold uppercase text-text-secondary tracking-wider">Sorteo</span>
                                <div class="relative">
                                    <select id="filter-sorteo"
                                        class="w-full h-11 pl-3 pr-10 bg-[#111318] border border-[#282d39] rounded-lg text-sm text-white focus:ring-2 focus:ring-primary/50 focus:border-primary appearance-none cursor-pointer">
                                        <option value="">Todos los sorteos</option>
                                        <?php foreach ($sorteosUnicos as $idSorteo => $titulo): ?>
                                            <option value="<?php echo $idSorteo; ?>"
                        ...
                        <div class="w-full lg:w-48">
                            <label class="flex flex-col gap-1.5 w-full">
                                <span
                                    class="text-xs font-semibold uppercase text-text-secondary tracking-wider">Estado</span>
                                <div class="relative">
                                    <select
                                        class="w-full h-11 pl-3 pr-10 bg-[#111318] border border-[#282d39] rounded-lg text-sm text-white focus:ring-2 focus:ring-primary/50 focus:border-primary appearance-none cursor-pointer">
                    ...
                    <?php if (empty($boletosCliente)): ?>
                        ...
                        <div class="col-span-full flex flex-col items-center justify-center py-16 px-4">
                            <div class="size-24 rounded-full bg-card-dark flex items-center justify-center mb-6">
                                <span
                                    class="material-symbols-outlined text-5xl text-text-secondary">confirmation_number</span>
                            </div>
                            <h3 class="text-2xl font-bold text-white mb-2">No tienes boletos aún</h3>
                            <p class="text-text-secondary text-center max-w-md mb-6">Cuando compres boletos en algún sorteo,
                                aparecerán aquí con su estado actualizado.</p>
                            <a href="ListadoSorteosActivos.php"
                                class="px-6 py-3 bg-primary hover:bg-blue-600 text-white font-bold rounded-lg transition-colors">
                    ...
                    <?php else: ?>
                        <?php foreach ($boletosCliente as $boleto):
                            ...
                            <div class="boleto-card group relative bg-card-dark rounded-xl <?php echo $borderClass; ?> overflow-hidden flex flex-col"
                                data-boleto-id="<?php echo $idBoleto; ?>" data-sorteo-id="<?php echo $idSorteo; ?>"
                                data-numero-boleto="<?php echo $numeroBoletoInt; ?>" data-estado="<?php echo $estadoDisplay; ?>"
                                data-sorteo-nombre="<?php echo $sorteoTitulo; ?>">
                                <?php if ($isWinner): ?>
                                    <div
                                        class="absolute top-0 right-0 bg-amber-500 text-black text-xs font-bold px-3 py-1 rounded-bl-lg z-10 flex items-center gap-1">
                                ...
                                <div class="p-5 flex items-start gap-4 border-b border-[#282d39] <?php echo $headerClass; ?>">
                                    <div class="size-16 rounded-lg bg-gray-800 shrink-0 overflow-hidden">
                                        <img alt="<?php echo $sorteoTitulo; ?>" class="w-full h-full object-cover"
                                    ...
                                    <div class="flex flex-col flex-1">
                                        ...
                                        <?php if (!$isWinner): ?>
                                            <div class="mt-2 inline-flex">
                                                <span
                                                    class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium <?php echo $badgeClass; ?>">
                                ...
                                <div class="p-5 flex flex-col gap-4 flex-1">
                                    <?php if ($estadoDisplay === 'rejected'): ?>
                                        <div class="bg-red-900/10 p-3 rounded-lg border border-red-900/30">
                                            <p class="text-xs text-red-400 leading-relaxed">
                                                <strong>Motivo:</strong> El comprobante de pago no es legible o no corresponde al
                                                monto del sorteo.
                                            </p>
                                        </div>
                                    <?php elseif ($estadoDisplay === 'pending' && empty($comprobanteUrl)): ?>
                                        <div
                                            class="flex justify-between items-center bg-[#111318] p-3 rounded-lg border border-dashed border-[#282d39] opacity-75">
                                        ...
                                        <div class="mt-auto flex gap-3">
                                            <div
                                                class="flex-1 h-10 flex items-center text-xs text-yellow-400 bg-yellow-900/10 rounded-lg px-3">
                                                Tu boleto se asignará al confirmar el pago.
                                            </div>
                                            <button
                                                class="size-10 rounded-lg border border-[#282d39] flex items-center justify-center hover:bg-[#353b4b] transition-colors text-text-secondary"
                                                title="Ayuda">
                                    ...
                                    <?php else: ?>
                                        <div
                                            class="flex justify-between items-center <?php echo $isWinner ? '' : 'bg-[#111318] p-3 rounded-lg border border-dashed border-[#282d39]'; ?>">
                                            <span
                                                class="text-sm font-medium text-text-secondary"><?php echo $isWinner ? 'Boleto #' : 'Número de Boleto'; ?></span>
                                            <span
                                                class="font-mono <?php echo $isWinner ? 'text-2xl font-bold tracking-wider text-amber-500' : 'text-xl font-bold text-white'; ?>"><?php echo $numeroBoleto; ?></span>
                                        </div>
                                        <div class="mt-auto pt-4 flex gap-3">
                                            <?php if ($isWinner): ?>
                                                <button
                                                    class="btn-reclamar flex-1 bg-amber-500 hover:bg-amber-600 text-black font-bold h-10 rounded-lg flex items-center justify-center gap-2 transition-colors text-sm"
                                            ...
                                            <?php elseif ($estadoDisplay === 'approved' && $comprobanteUrl): ?>
                                                <button
                                                    class="btn-comprobante flex-1 bg-[#282d39] border border-[#282d39] hover:border-primary hover:text-primary text-gray-300 font-medium h-10 rounded-lg flex items-center justify-center gap-2 transition-colors text-sm"
                                            ...
                                            <?php elseif ($estadoDisplay === 'rejected'): ?>
                                                <button
                                                    class="btn-soporte flex-1 bg-[#282d39] border border-[#282d39] hover:border-red-500 hover:text-red-500 text-gray-300 font-medium h-10 rounded-lg flex items-center justify-center gap-2 transition-colors text-sm"
                                                    data-sorteo-titulo="<?php echo $sorteoTitulo; ?>">
                                                    Contactar Soporte
                                                </button>
                                            <?php endif; ?>
                                            <button
                                                class="btn-ver-detalles size-10 rounded-lg border border-[#282d39] flex items-center justify-center hover:bg-[#353b4b] transition-colors text-text-secondary"
                                                data-boleto-id="<?php echo $idBoleto; ?>"
                                                data-numero-boleto="<?php echo $numeroBoleto; ?>"
                                                data-sorteo-titulo="<?php echo $sorteoTitulo; ?>"
                                                data-fecha="<?php echo $transaccionFecha; ?>"
                                                data-estado-texto="<?php echo $estadoTexto; ?>" title="Ver detalles">
                                                <span
                                                    class="material-symbols-outlined"><?php echo $estadoDisplay === 'pending' && empty($comprobanteUrl) ? 'help' : 'visibility'; ?></span>
                                            </button>
                                        </div>
                                    <?php endif; ?>
                                </div>
                ...
                <div class="flex items-center justify-center gap-2 pt-4">
                    <button
                        class="flex items-center justify-center size-10 rounded-lg border border-[#282d39] text-text-secondary hover:bg-[#353b4b] disabled:opacity-50 transition-colors">
                        <span class="material-symbols-outlined">chevron_left</span>
                    </button>
                    <button
                        class="flex items-center justify-center size-10 rounded-lg bg-primary text-white font-medium hover:bg-blue-600 transition-colors">1</button>
                    <button
                        class="flex items-center justify-center size-10 rounded-lg border border-[#282d39] text-text-secondary hover:bg-[#353b4b] transition-colors">2</button>
                    <button
                        class="flex items-center justify-center size-10 rounded-lg border border-[#282d39] text-text-secondary hover:bg-[#353b4b] transition-colors">3</button>
                    <span class="text-text-secondary px-1">...</span>
                    <button
                        class="flex items-center justify-center size-10 rounded-lg border border-[#282d39] text-text-secondary hover:bg-[#353b4b] transition-colors">
    ...
    <script>
        ...
        function initLogoutButton() {
            ...
            if (logoutBtn) {
                ...
                newLogoutBtn.addEventListener('click', function(e) {
                    ...
                    // Usar ClientLayout si está disponible
                    if (window.ClientLayout && typeof window.ClientLayout.handleLogout === 'function') {
                        window.ClientLayout.handleLogout();
                    } else if (window.handleGlobalLogout && typeof window.handleGlobalLogout === 'function') {
        ...
        function initBoletoActions() {
            ...
            comprobanteButtons.forEach(button => {
                button.addEventListener('click', function () {
                    const card = this.closest('.grid > div'); // Ajustar selector si es tarjeta
                    // Intentar obtener ID del boleto
                    let boletoId = card?.getAttribute('data-id-boleto');

                    // Si no está en la tarjeta, buscar en el botón mismo (si lo agregamos)
                    if (!boletoId) boletoId = this.getAttribute('data-id-boleto');

                    // Si aún no tenemos ID, intentar obtenerlo del contexto (esto requiere que el PHP lo imprima)
                    // Por ahora, asumiremos que se agregará data-id-boleto al HTML

                    if (boletoId) {
                        window.open(`comprobante.php?boleto=${boletoId}`, '_blank');
                    } else {
                        // Fallback: Si no hay ID, mostrar error o intentar con número (si el backend lo soporta)
                        const boletoNumber = card?.querySelector('.font-mono')?.textContent.replace('#', '').trim();
                        if (boletoNumber) {
                            // Opción B: Modificar comprobante.php para aceptar numero_boleto también, o...
                            // Mejor: Mostrar alerta de que falta implementación de ID en frontend
                            customAlert('Error: No se pudo identificar el ID del boleto para generar el comprobante through ID. Intentando por número...', 'Error', 'warning');
                            // window.open(`comprobante.php?numero=${boletoNumber}`, '_blank');
                        } else {
                            customAlert('No se pudo generar el comprobante.', 'Error', 'error');
                        }
                    }
                });
            });
