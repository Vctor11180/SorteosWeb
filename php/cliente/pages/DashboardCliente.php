...
if (!isset($_SESSION['is_logged_in']) || $_SESSION['is_logged_in'] !== true) {
    header('Location: ../auth/InicioSesion.php?rol=cliente');
    exit;
}

// Verificar que tenga los datos mínimos en la sesión
if (!isset($_SESSION['usuario_id']) || !isset($_SESSION['usuario_email'])) {
    // Limpiar sesión inválida
    $_SESSION = array();
    session_destroy();
    session_start();
    header('Location: ../auth/InicioSesion.php?rol=cliente&error=sesion_invalida');
    exit;
}
...
// Si no se pueden obtener los datos, limpiar sesión y redirigir al login
if (!$datosUsuario) {
    error_log("DashboardCliente - No se pudieron obtener datos del usuario ID: " . ($_SESSION['usuario_id'] ?? 'N/A'));
    $_SESSION = array();
    session_destroy();
    session_start();
    header('Location: ../auth/InicioSesion.php?rol=cliente&error=datos_no_encontrados');
    exit;
}
...
async function fetchClientDataFromServer(baseClientData) {
    try {
        ...
        const response = await fetch('../api/api_dashboard.php?action=get_stats');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Inicializar botón de logout (con prioridad)
    initLogoutButton();
    ...
    // También asegurar que el logout funcione después de que se cargue ClientLayout
    setTimeout(function() {
        initLogoutButton();
    }, 500);
});
